...
impl MultiShell {
    ...
    pub fn verbose<'a, F>(&mut self, callback: F) -> IoResult<()> where
        F: Fn<&'a mut MultiShell, IoResult<()>>
    {
        if self.verbose { return callback(self) }
        Ok(())
    }

    pub fn concise<'a, F>(&mut self, callback: F) -> IoResult<()> where
        F: Fn<&'a mut MultiShell, IoResult<()>>
    {
        if !self.verbose { return callback(self) }
        Ok(())
